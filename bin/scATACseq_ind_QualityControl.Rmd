...
# Quality control metrics on scATACseq 
...
for (i in 1:number_condition) {
  ...
  list_graphs[[names(list_seurat_obj[i])]] = list("qc_plot_nucl_fragments" = temp$qc_plot_nucl_fragments,
...
```{r, Visualize QC metrics, results = 'hold', fig.width=18, fig.height=12}
...
```{r, Save outputs with QC metrics}
...
saveRDS(object = list_seurat_obj, file = paste0(directory_output, "list_seurat_obj_annot_qc_filtered.rds"))

```

```{r, Rsession}

# Clean working space and show package versioning
rm(list = ls())
sessionInfo()
